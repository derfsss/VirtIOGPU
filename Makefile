...
# PRE-GENERATED by InstallerScriptGen from
# installer/virtiogpu_installer_fixture.py (regeneration instructions in
# that file's docstring).  They are committed so dist needs no extra
# tooling.  On the Amiga the archive is installed by double-clicking
